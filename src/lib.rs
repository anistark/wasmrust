...
impl WasmRustPlugin {
    ...
    fn compile_with_trunk(&self, config: &CompileConfig) -> Result<CompileResult> {
        ...
        if config.verbose {
            println!("Running: trunk {} (from directory: {})", args.join(" "), config.project_path);
        ...
        if config.verbose {
            println!("Checking for index.html at: {}", index_in_project_dist.display());
        ...
        if index_in_project_dist.exists() {
            ...
            if project_dist != Path::new(&config.output_dir) {
                fs::create_dir_all(&config.output_dir)?;
                self.copy_dir_recursive(&project_dist, Path::new(&config.output_dir))?;
            }
        ...
        if !index_path.exists() {
            return Err(WasmRustError::CompilationFailed(
                format!("No index.html generated by trunk. Checked: {} and {}", 
